    ...
    def update_intermediate_goal(self, event=None):
        ...
        try:
            if self.original_goal.header.frame_id != self.costmap.header.frame_id:
                # timestamp를 현재 시간으로 업데이트하여 최신 TF 사용
                goal_with_current_time = PoseStamped()
                goal_with_current_time.header = self.original_goal.header
                goal_with_current_time.header.stamp = rospy.Time(0)  # 최신 TF 사용
                goal_with_current_time.pose = self.original_goal.pose
                goal_in_costmap_frame = self.tf_buffer.transform(goal_with_current_time, self.costmap.header.frame_id, rospy.Duration(1.0))
            else:
